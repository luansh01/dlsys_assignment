...
class NDArray:
    ...
    def reshape(self, new_shape):
        ...
        ### BEGIN YOUR SOLUTION
        if not self.is_compact() and not prod(self.shape) == prod(new_shape):
            raise ValueError()
        else:
            new_strides = self.compact_strides(new_shape)
            return  NDArray.make(tuple(new_shape), tuple(new_strides), self._device, self._handle)
        ### END YOUR SOLUTION
    ...
    def permute(self, new_axes):
        ...
        ### BEGIN YOUR SOLUTION
        original_dims = len(self._shape)
        new_shape = [None] * original_dims
        new_strides = [None] * original_dims
        if len(new_axes) != original_dims:
            raise ValueError()
        if len(set(new_axes)) != original_dims:
            raise ValueError()
        if any(d >= original_dims or d < 0 for d in new_axes):
            raise ValueError()
    ...
    def __getitem__(self, idxs):
        ...
        new_strides = [None] * len(self._shape)
        new_shape = [None] * len(self._shape)
