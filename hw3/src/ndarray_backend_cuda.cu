...
__global__ void CompactKernel(const scalar_t* a, scalar_t* out, size_t size, CudaVec shape,
  ...
  if(gid < size){
    ...
    size_t curr_len = 0, pre_len = 1; 
    for(int i = shape.size - 1;i>=0;i--){
      curr_len = pre_len * shape.data[i];
      idxs[i] = gid % curr_len / pre_len;
      pre_len = curr_len;
    }
    size_t loc = offset;
    for(int i = 0;i<shape.size;i++){
      loc += idxs[i] * strides.data[i];
    }
    out[gid] = a[loc];
  }
  /// END SOLUTION
...
__global__ void EwiseSetitemKernel(const scalar_t* a, scalar_t* out, size_t size, CudaVec shape,
                                    CudaVec strides, size_t offset ){
    size_t gid = blockIdx.x * blockDim.x + threadIdx.x;
    //const scalar_t* start_ptr = a + offset;
    if(gid < size){
    size_t idxs[MAX_VEC_SIZE];
    size_t curr_len = 0, pre_len = 1; 
    for(int i = shape.size - 1;i>=0;i--){
...
__global__ void MatmulKernel(const scalar_t* mat1, const scalar_t* mat2, scalar_t* out, uint32_t M, uint32_t N,
            uint32_t P){
...
void Matmul(const CudaArray& a, const CudaArray& b, CudaArray* out, uint32_t M, uint32_t N,
  ...
  dim3 grid_size = dim3((M+15)/16, (P+15)/16);
  MatmulKernel<<<grid_size, block_size>>>(a.ptr, b.ptr, out->ptr,M,N,P);
  /// END SOLUTION
}
...
void ReduceMax(const CudaArray& a, CudaArray* out, size_t reduce_size) {
   ...
   */
  /// BEGIN SOLUTION
  dim3 block_dim = dim3(out->size);
  dim3 grid_dim = dim3(1);
  ReduceMaxKernel<<<grid_dim,block_dim>>>(a.ptr, out->ptr, reduce_size, out->size);
  /// END SOLUTION
}
...
void ReduceSum(const CudaArray& a, CudaArray* out, size_t reduce_size) {
  ...
  dim3 block_dim = dim3(out->size);
  dim3 grid_dim = dim3(1);
  ReduceSumKernel<<<grid_dim,block_dim>>>(a.ptr, out->ptr, reduce_size, out->size);
  /// END SOLUTION
}
